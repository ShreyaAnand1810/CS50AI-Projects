...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            ...
            # Make a copy of x's domain to iterate safely
            for x_word in self.domains[x].copy():
                # Check if there exists a y_word that matches the overlap condition
                if not any(
    ...
    def ac3(self, arcs=None):
        ...
        """
         # Initialize queue with provided arcs or all arcs
        ...
        while queue:
            x, y = queue.pop(0)
            if self.revise(x, y):
                if not self.domains[x]:  # Domain of x is empty
                    return False
